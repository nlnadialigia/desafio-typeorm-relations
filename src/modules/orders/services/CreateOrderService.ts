...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductWithoutEnoughQuantity = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity < product.quantity
    )

    if (findProductWithoutEnoughQuantity.length) {
      throw new AppError(`There is not enought ${findProductWithoutEnoughQuantity[0].id}`);
    }
